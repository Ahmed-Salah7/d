@extends('layouts.app')
@section('title') My tickets @endsection
@section('content') 
<!-- Page-Title -->
<div class="row">
    <div class="col-sm-12">
        <div class="page-title-box">
            <div class="row align-items-center">
                <div class="col-md-8">
                    <h4 class="page-title m-0">{{ __('page.my_tickets') }}</h4>
                </div>
                <div class="col-md-4">
                    <div class="float-right">
                        <div class="dropdown">
                            <button class="btn btn-primary add-ticket" type="button" data-title="{{ __('page.add_ticket') }}">
                                <i class="ti-plus mr-1"></i> {{ __('page.add_ticket') }}
                            </button>
                        </div>
                    </div>
                </div>
        ...
        <div class="card m-b-30">
            ...
            <div class="card-body">
            	<ul class="nav nav-tabs" role="tablist">
				    <li class="nav-item">
				        <a class="nav-link active" data-toggle="tab" href="#tab_1" aria-expanded="true">{{ __('page.opened') }} <span class="badge badge-warning opened_status">{{ openedStatus('my_ticket') }}</span></a>
				    </li>
				    <li class="nav-item">
				        <a class="nav-link" data-toggle="tab" href="#tab_2">{{ __('page.closed') }} <span class="badge badge-success close_status">{{ closedStatus('my_ticket') }}</span></a>
				    </li>
				</ul>
...
<script type="text/javascript">
	$( document ).ready( function(){
        ...
        $('#openstatus').DataTable({
             ...
             ordering: false,
             ajax: {
             url: baseUrl+"/support/ticket/status-open-my-ticket" ,
              type: 'GET',
             },
             columns: [
        ...
        $('#closestatus').DataTable({
            ...
            ordering: false,
            ajax: {
                url: baseUrl+"/support/ticket/status-close-my-ticket" ,
                type: 'GET',
            },
            columns: [
                    { data: 'subject', name: 'subject' },
		...
		//ticket code
	    $( document ).on( 'click',".add-ticket",function( event ) {
    ...
    show: true
	        ...
	        $.ajax({
	            url: baseUrl+"/support/add-ticket",
	            type:'GET',
        ...
        $( document ).on( "submit","#add_comment",function( event ) {
            ...
            formdata.append('page','my_ticket');
            errors = ticketValidation(formObj,errors);
    ...
    show: true
        ...
        $( document ).on( "click",".ticket_colse",function( event ) {
            var id  = $(this).data('id');
            var formdata = {'id':id,'page':'my_ticket'};
            if( id ){
                $.ajax({
    ...
    //remove error class
        ...
        //find flields 
        var subjectObj = formObj.find('input[name="subject"]');
        ...
        if( subject =="" ) {
            errors = 1;
            subjectObj.addClass('parsley-error');
        }
        if( message =="" ) {
            errors = 1;
