...
@section('content')
        ...
        <div class="col-sm-12">
            <div class="page-title-box">
                <div class="row align-items-center">
                    <div class="col-md-8">
                        <h4 class="page-title m-0">{{ __('page.transfer_sponsorship_requests_list') }}</h4>
                    </div>
                    <div class="col-md-4">
                        <div class="float-right">
                            <div class="dropdown">
                                <button class="btn btn-primary add-transfer_sponsorship_request" type="button" data-title="{{ __('page.add_transfer_sponsorship_request') }}">
                                    <i class="ti-plus mr-1"></i> {{ __('page.add_transfer_sponsorship_request') }}
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card-body">
                    ...
                    <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                        ...
                        <tr>
                            ...
                            <th> {{ __('page.customer_current') }}</th>
                            <th> {{ __('page.customer_new') }}</th>
                            <th> {{ __('page.date_transfer_sponsorship') }}</th>
                            <th> {{ __('page.cost_transfer_sponsorship') }}</th>
                            <th> {{ __('page.expiration_date_experiment') }}</th>
    ...
    <div class="modal fade bs-example-modal-lg transfer_sponsorship_request-modal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
...
@section('js')
    <script type="text/javascript">
        ...
        $( document ).ready( function(){
            ...
            var under_experiment = searchParams.get('under_experiment');
            ...
            $('#datatable').DataTable({
                ...
                ordering: false,
                ajax: {
                    url: baseUrl+"/transfer-sponsership-request?under_experiment="+under_experiment,
                    type: 'GET',
                    data: function (d) {
                ...
                columns: [
                    ...
                    { data: 'customer_current', name: 'customer_current' },
                    { data: 'customer_new', name: 'customer_new' },
                    { data: 'date_transfer_sponsorship', name: 'date_transfer_sponsorship' },
                    { data: 'cost_transfer_sponsorship', name: 'cost_transfer_sponsorship' },
                    { data: 'expiration_date_experiment', name: 'expiration_date_experiment' },
                    { data: 'action', name: 'action', orderable: false },
                ],
                order: [[0, 'desc']]
            });
            //transfer_sponsorship_request code
            $( document ).on( 'click',".add-transfer_sponsorship_request",function( event ) {
                var title = $(this).data('title');
                $('.modal-title').text(title);
                $(".modal-spinner").show();
                $('.result').html("");
                $(".transfer_sponsorship_request-modal").modal({
                    backdrop: 'static',
                    keyboard: true,
                    show: true
                });
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            if($.isEmptyObject(data.error)){
                                ...
                                oTable.fnDraw(false);
                                formObj[0].reset();
                                $(".transfer_sponsorship_request-modal").modal('hide');
            ...
            $( document ).on( 'click',".edit-transfer_sponsorship_request",function( event ) {
                var title = $(this).data('title');
                $('.modal-title').text(title);
                $(".modal-spinner").show();
                $('.result').html("");
                ...
                var id = $(this).data('id');
                $.ajax({
                    url: baseUrl+"/transfer-sponsership-request/"+id+"/edit",
                    type:'GET',
                    success: function(data) {
                        $(".modal-spinner").hide();
                        $('.result').html(data.view);
            ...
            $( document ).on( "click",".delete-transfer_sponsorship_request",function( event ) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: baseUrl+"/delete-transfer-sponsership-request",
                            type: "POST",
            ...
            $( document ).on( "submit","#update_transfer_sponsorship_request",function( event ) {
                event.preventDefault();
                $(".print-error-msg").css('display','none');
                $(".print-success-msg").css('display','none');
                //define variation
                ...
                if( errors == 0 ) {
                    ...
                    btnObj.find("i").addClass('fa-spinner fa-spin');
                    $.ajax({
                        url: baseUrl+"/transfer-sponsership-request/"+id,
                        type:'POST',
                        data: new FormData(this),
                        processData: false,
                        contentType: false,
